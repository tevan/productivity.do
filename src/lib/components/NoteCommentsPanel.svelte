<script>
  // Comments overlay for a note. Shares the same anchored-overlay pattern
  // as RevisionHistoryPanel so the host modal stays simple. Author-only:
  // every comment in this view belongs to the current user. Shape will
  // generalize when sharing ships and recipients can post.
  ...
  async function load() {
    loading = true; err = '';
    const r = await api(`/api/notes/${noteId}/comments`);
  ...
  async function saveEdit(c) {
    ...
    if (!body) return;
    busy = true;
    const r = await api(`/api/notes/${noteId}/comments/${c.id}`, {
      method: 'PUT',
      body: JSON.stringify({ body }),
    });
    busy = false;
  ...
  async function remove(c) {
    if (!await confirmAction({
      ...
      body: 'You can\'t undo this from here.',
      confirmLabel: 'Delete',
    })) return;
    busy = true;
    const r = await api(`/api/notes/${noteId}/comments/${c.id}`, { method: 'DELETE' });
    busy = false;
    if (r?.ok) {
  ...
  function onKeydown(e) {
    // Cmd/Ctrl+Enter posts the draft.
      ...
      {:else}
        <ul class="cmt-list">
          {#each comments as c (c.id)}
            <li class="cmt-row">
              <div class="cmt-meta">
                <span class="when" title={full(c.createdAt)}>{relative(c.createdAt)}</span>
                {#if c.updatedAt && c.updatedAt !== c.createdAt}
                  <span class="edited" title={full(c.updatedAt)}>(edited)</span>
                {/if}
              ...
              {:else}
                <p class="cmt-body-text">{c.body}</p>
              {/if}
            </li>
          {/each}
        </ul>
...
<style>
  ...
  .cmt-meta .edited { font-style: italic; }
  .cmt-actions {
    margin-left: auto;
