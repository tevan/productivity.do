...
→ See **`docs/LAUNCH-CHECKLIST.md`** for the canonical list of things the human needs to do before public launch (Stripe / Resend / Sentry / Anthropic / Cloudflare SSL / Google OAuth verification / charter-user list / site-gate removal / post-launch monitoring / rollback plan). Don't duplicate items here — keep them in one place.

## Recently shipped (2026-05-02)

- ✓ Schema-complete `/api/account/export` (every user-owned table).
- ✓ Soft-delete email collision fix (`original_email` column + rename-on-delete + match-both-on-login).
- ✓ Tenancy audit (links DELETE atomic, booking-pages PUT scoped).
- ✓ Booking widget acquisition CTAs (`/book/:slug`, `/q/:id`, cancel/reschedule).
- ✓ Stripe configuration gate (refuses checkout when prices unset).
- ✓ Full WCAG 2.1.1 keyboard support across calendar grid + chips (roving tabindex, slot/event focus modes, aria-live announcements, EventEditor / EventChip / TaskRow / MonthView all updated).
