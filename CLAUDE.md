...
Calendar / events (protected):
  ...
  + recurrence on POST/PUT (preset, RRULE, or {freq,interval,count,until,byDay,byMonthDay});
    ...
    DELETE accepts ?scope=instance|series|following (default instance)
...
- **Recurring events:** `expandRecurrence()` in `backend/routes/calendar.js` translates `'daily'|'weekdays'|'weekly'|'biweekly'|'monthly'|'yearly'` presets, raw `RRULE:` strings, or `{freq,interval,count,until,byDay,byMonthDay}` objects into Google's `recurrence[]`. Both PUT and DELETE accept `scope`: `instance` (default), `series` (resolves `recurringEventId`, patches/deletes parent), or `following` (terminates parent series with `UNTIL=instance-1s` — and on PUT, creates a fresh series from the instance forward). The shared scope math lives in `backend/lib/recurrence.js` (`computeUntilForFollowing`, `applyUntilToRecurrenceLines`, `normalizeScope`) with tests at `recurrence.spec.js`. Frontend prompts the user via `chooseRecurrenceScope()` from `src/lib/utils/chooseScopeModal.svelte.js` whenever the action targets an instance whose `recurringEventId` is set; `<ScopeModalRoot>` mounted in `App.svelte` renders the dialog. Both delete entry points (right-click `EventContextMenu` and keyboard delete in `TimeGrid`) and edit flows go through this picker. Default-selected option is `instance` — the safest scope.
...
**Thumbnails (server-side via `sharp`):** Image uploads (jpeg/png/webp/gif/avif/tiff) get a 256px-longest-edge webp thumbnail at `<files-dir>/<hash[0:2]>/<hash>.thumb.webp` generated on first upload. The pipeline calls `.rotate()` before `.resize()` so EXIF-oriented photos render upright; output bytes have orientation stripped. SVG is intentionally excluded — vector → raster resizing loses fidelity, and SVGs are small enough to serve as-is. Serve via `GET /api/files/:id?thumb=1` (falls back to the original if the thumb is missing — older uploads, non-images, or generation failure). API row carries `thumbUrl` only when the thumb file actually exists on disk; null otherwise. DELETE GCs the thumb alongside the blob. Test invariants in `backend/routes/files.thumb.spec.js`.
